	...
	<Container maxWidth="container.xxl" bg='#f8f9d'>
		...
		<Flex h="full" p='7% 20% 5% 15%' marginLeft="3vw" direction={{ base: "column", md: "row" }}>
			<VStack spacing={4}>
				<HStack w='70vw' justifyContent='space-between'>
					<VStack alignItems='flex-start'>
						<HStack>
							<Heading size="xl" marginRight='1.5vw'>{hotelName}</Heading>
          ...
          <Grid column={3} columnGap={3} rowGap={3} w="full" h='3vw' marginTop='3vw' color='black.main'>
            ...
            <GridItem colSpan={1} textAlign='center' paddingTop={0}> <Heading fontSize='xl'>Areas Progress</Heading></GridItem>
        ...
        <Grid templateColumns='repeat(3, 1fr)'
            ...
            {areasMapped.map((area:string, index:number) => (
                <GridItem>
                  <VStack p={5} shadow='md' borderWidth='1px'  w='22vw' borderRadius='lg'>
                    <Heading fontSize='xl' color='black.main' fontWeight='bold' marginTop='1vw' marginBottom='2vw'>{area}</Heading>
                    <CircularProgress value={percentagePerMappedArea[index]} size='120px'>
                      <CircularProgressLabel color='#2F6FE4' fontSize='lg' fontWeight='bolder'>{percentagePerMappedArea[index]}%</CircularProgressLabel>
                    </CircularProgress>
                  </VStack>
                </GridItem>
              )
        ...
        <HStack alignItems='left' width='100%'>
          <Text textAlign='left' color='black.500' size='lg' fontWeight='bold'>Show non completed areas</Text>
          <Switch size='lg'/>
        </HStack>
